...
async function axValidate(axText) {
    const chordDic = new Map();
    const allowAnnotations = [];
    const mapSeed = {};
    const res = Conductor.convertToObj(false, false, axText, allowAnnotations, chordDic, mapSeed);
    if (res.error) {
        throw { details: [{ message: res.error.message, line: res.error.line, column: res.error.linePos, token: res.error.token }] };
    }
}
async function compile(axText, _options) {
    const chordDic = new Map();
    const allowAnnotations = [];
    const mapSeed = {};
    const res = Conductor.convertToObj(true, true, axText, allowAnnotations, chordDic, mapSeed);
    if (res.error) {
        throw { details: [{ message: res.error.message, line: res.error.line, column: res.error.linePos, token: res.error.token }] };
    }
...
const midiPath = (hash) => path.join(DIR, `${hash}.mid`);
// Ensure data directory exists
async function ensureDir() {
    await fs.mkdir(DIR, { recursive: true });
}
// Simple input validators
function ensureString(v, name, opts) {
    if (typeof v !== "string")
...
async function compileToMidi(axText) {
    ...
    try {
        ...
        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("Compilation timeout")), COMPILE_TIMEOUT_MS));
